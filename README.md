...
- **Publishers page** – The Publishers link shows reach and region data loaded from Supabase. The list is read-only and feeds the automated reach numbers. Ensure your Supabase project includes a table named `publishers` (or `publisher`) with columns `Network_Publishers`, `Sub_Publishers`, `Status`, `Regions` and `Reach`.
