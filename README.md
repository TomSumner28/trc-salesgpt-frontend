...
- **Saved forecasts** – After calculating results you can save the current forecast. Entries are cached locally so they appear instantly, then synced to your Supabase `forecasts` table (configure `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY`). The Saved Forecasts page lists every entry so anyone can review or delete them later.
