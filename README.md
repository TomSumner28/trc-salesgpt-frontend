...
- **Publisher forecasts** – Use the Publisher Forecasts page to enter real transaction count and revenue figures from partners. Cashback is entered as a percentage so totals are calculated automatically. Results include AOV and ROAS and can cover a 3‑ or 6‑month period in any currency. The results dropdown lets you view totals by offer type or by channel, and the high level metrics list total cashback for existing and new customers.
